    ...
    <div class="min-h-screen flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8 relative bg-slate-50/70">
      ...
      <div class="max-w-md w-full relative z-10 space-y-6">
        ...
        <div class="text-center space-y-2">
          ...
          <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Verifikasi Akun</h1>
        ...
        <div class="bg-white p-7 sm:p-9 rounded-3xl border border-slate-200/80 shadow-ambient-lg">
          <form class="space-y-5" [formGroup]="otpForm" (ngSubmit)="onVerify()">
            
            <!-- Email (Hidden or Read-only when available) -->
            ...
            <!-- OTP Input -->
            <div class="space-y-2">
              <label for="otp" class="block text-xs font-bold text-slate-700 text-center">Kode OTP 6 Digit</label>
              <div class="relative">
                <input id="otp" type="text" formControlName="otp" maxlength="6" autocomplete="one-time-code"
                  placeholder="• • • • • •"
                  ...
                  [ngClass]="{'border-rose-400 focus:ring-rose-100 focus:border-rose-400': submitted && f['otp'].errors}">
              </div>
              <div *ngIf="submitted && f['otp'].errors" class="text-rose-600 text-[11px] font-medium text-center pt-0.5">
                <span *ngIf="f['otp'].errors['required']">Kode OTP wajib diisi</span>
                <span *ngIf="f['otp'].errors['pattern']">Kode OTP harus berupa 6 digit angka</span>
              </div>
            </div>
            ...
            <div class="pt-2">
              <button type="submit" [disabled]="isLoading"
                ...
                <span>{{ isLoading ? 'Memverifikasi...' : 'Verifikasi Akun' }}</span>
              </button>
            </div>
            ...
            <div class="pt-2 text-center border-t border-slate-100">
              <p class="text-xs text-slate-500">
                ...
                <button type="button" (click)="onResend()" [disabled]="isResending || resendCountdown > 0"
                  ...
                  <span *ngIf="resendCountdown === 0">{{ isResending ? 'Mengirim...' : 'Kirim Ulang OTP' }}</span>
                </button>
              </p>
            </div>
        ...
        <div class="flex justify-between items-center text-xs text-slate-500 px-2">
          <a routerLink="/register" class="hover:text-slate-800 transition-colors">
            &larr; Ubah Data Registrasi
          </a>
          <a routerLink="/login" class="hover:text-slate-800 transition-colors">
            Sudah Verifikasi? Masuk &rarr;
          </a>
        </div>
...
export class VerifyOtpComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      if (params['email']) {
        ...
        this.otpForm.patchValue({ email: this.targetEmail });
      }
    });
  ...
  onOtpInput(event: Event): void {
    ...
    this.otpForm.patchValue({ otp: input.value });
  }
    ...
    this.authService.verifyOtp(payload).subscribe({
      next: (res) => {
        ...
        this.toastService.success(
          'Verifikasi Berhasil',
          res?.message || 'Akun Anda berhasil diverifikasi. Silakan login.'
        );
        this.router.navigate(['/login']);
      },
      error: (err) => {
        this.isLoading = false;
        const msg = err?.error?.message || err?.message || 'Verifikasi gagal. Pastikan kode OTP benar.';
  ...
  onResend(): void {
    const email = this.otpForm.get('email')?.value || this.targetEmail;
    if (!email) {
      this.toastService.error('Error', 'Alamat email wajib diisi untuk kirim ulang OTP.');
      return;
    }

    this.isResending = true;
    this.authService.resendOtp({ email: email.trim() }).subscribe({
      next: (res) => {
        this.isResending = false;
        this.toastService.success('OTP Terkirim', res?.message || 'Kode OTP baru telah dikirimkan ke email Anda.');
        this.startCooldown();
      },
      error: (err) => {
        this.isResending = false;
        const msg = err?.error?.message || err?.message || 'Gagal mengirim ulang kode OTP.';
        this.toastService.error('Gagal Mengirim OTP', msg);
      }
    });
  }
}
